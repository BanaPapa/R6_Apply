import { supabaseAdmin } from '../supabase/serverClient.mjs';
...
export async function archivePage(enriched, supplyAreaCode) {
  if (!supabaseAdmin || !Array.isArray(enriched) || enriched.length === 0) return;
  try {
...
export async function archiveDetail(houseManageNo, pblancNo, detail) {
  if (!supabaseAdmin || !detail) return;
  try {
    const { error } = await supabaseAdmin
      .from('apply_announcements')
...
function fromAnnouncementRow(r) {
  return {
    ...
    averageCompetitionRate: r.average_competition_rate || 0,
    maxCompetitionRate: r.max_competition_rate || 0,
